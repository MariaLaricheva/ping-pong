...
// Задаём максимальное расстояние, на которое может подняться платформа игрока
...
// Скорость платформы
var paddleSpeed = 6;
// Скорость мяча (медленнее чем в мультиплеере)
var ballSpeed = 4;
...
function loop() {
    ...
    ball.x += ball.dx;
    ball.y += ball.dy;
    // пусть левая платформа движется так же, как и мяч
    ...
    // А теперь следим за тем, когда игрок отпустит клавишу, чтобы остановить движение платформы
    ...
    scoreDisplay.innerText = 'Счёт: ' + count;
}
